/**
 * Vue 生态层：remoteComponent() 远程组件直渲染 + createHostPages() 宿主页面适配器。
 *
 * 定位：runtime.js 保持框架无关（不 import vue，gzip 红线守卫），
 * Vue 封装按需引入——独立文件、零 runtime 体积增量。
 *
 * remoteComponent 语义（用法与选项见 README §8）：
 ...
 *
 * createHostPages 语义（用法见 README §10）：宿主页面表唯一来源 + URL 解析 +
 * 组件缓存 + 骨架屏/错误占位 + 保活名称；加载经 loadRemote，setup/onSession
 * 生命周期与 remoteComponent 完全同源。
 */
