...
describe('AppContext：provide merge 幂等', () => {
  it('两次 provide 叠加，后写覆盖同键，先写键保留', () => {
    provideAppContext({ user: { id: '101' }, locale: 'zh-cn' })
    provideAppContext({ locale: 'en', baseUrl: '/demo' })
  ...
  it('同键重复 provide 幂等（值相同不抖动）', () => {
    provideAppContext({ formUrl: '/flowable' })
    provideAppContext({ formUrl: '/flowable' })
    expect(getAppContext().formUrl).toBe('/flowable')
  })
})
...
describe('AppContext：get 快照（传输层，嵌套引用共享）', () => {
  it('嵌套对象（events）引用共享：写入方挂属性即时可见', () => {
    ...
    provideAppContext({ events: { main: mainEvents } })
    const ctx = getAppContext()
...
describe('AppContext：require 显式校验', () => {
  it('键齐全时返回整个 context（调用方解构消费）', () => {
    provideAppContext({ store: { state: 1 }, user: { id: 1 }, hostApp: { use: () => {} } })
    const ctx: AppContext = requireAppContext('store', 'user', 'hostApp')
    expect(ctx.store).toEqual({ state: 1 })
    expect(ctx.user).toEqual({ id: 1 })
  })

  it('缺任一键 → CC-001 三段式（错误码 + got/expected + 修法指向宿主桥），零静默', () => {
    provideAppContext({ locale: 'zh-cn' })
    try {
      requireAppContext('store', 'user', 'hostApp')
      expect.unreachable('requireAppContext should have thrown')
    } catch (err: any) {
...
describe('AppContext：CC-002 无运行时（独立直开远程页）', () => {
  ...
  it('globalThis.__FULGURJS_RUNTIME__ 缺失时 provide/get/require 全部 CC-002 显式抛错', () => {
    ...
    for (const fn of [
      () => provideAppContext({ user: {} }),
      () => getAppContext(),
      () => requireAppContext('store'),
    ]) {
      try {
        fn()
...
describe('AppContext：globalThis 守卫防多副本', () => {
  it('context 模块被重新实例化（多 bundle 各带一份）仍读写同一全局存储', async () => {
    provideAppContext({ baseUrl: '/demo' })
    vi.resetModules()
    const fresh = await import('../src/context')
    // 新副本 provide → 旧副本 get 可见；新副本 require 也命中（委托同一 runtime 单例）
    fresh.provideAppContext({ formUrl: '/form' })
    expect((getAppContext() as any).baseUrl).toBe('/demo')
    expect((getAppContext() as any).formUrl).toBe('/form')
    expect(fresh.requireAppContext('formUrl').formUrl).toBe('/form')
  })
})
