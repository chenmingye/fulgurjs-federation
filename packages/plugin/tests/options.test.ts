...
function norm(opts: UnifedOptions) {
...
describe('options: remotes 解析（webpack 语法 + 单地址自动切换）', () => {
  ...
  it('键重命名语义（checkout: shop@...）', () => {
    const n = norm({
      name: 'host',
      remotes: { checkout: 'shop@http://localhost:3001/unifed-remoteEntry.js' },
    })
    expect(n.remotes[0].key).toBe('checkout')
    expect(n.remotes[0].name).toBe('shop')
  })
  ...
  it('单个 base 地址自动切换 dev/prod', () => {
    ...
    expect(n.remotes[0].devEntry).toBe('http://localhost:5101/@unifed-entry.js')
    expect(n.remotes[0].prodEntry).toBe('http://localhost:5101/unifed-remoteEntry.js')
  })

  it('带路径 base（微前端子应用场景）', () => {
    const n = norm({ name: 'host', remotes: { bpm: 'http://localhost:4529/flowable' } })
    expect(n.remotes[0].devEntry).toBe('http://localhost:4529/flowable/@unifed-entry.js')
    expect(n.remotes[0].prodEntry).toBe('http://localhost:4529/flowable/unifed-remoteEntry.js')
  })
  ...
  it('dev/prod 显式拆分', () => {
    ...
    expect(n.remotes[0].devEntry).toBe('http://localhost:5101/@unifed-entry.js')
    expect(n.remotes[0].prodEntry).toBe('https://cdn.example.com/ra/unifed-remoteEntry.js')
  })
...
describe('options: exposes / 其余选项', () => {
  ...
  it('filename 默认值', () => {
    expect(norm({ name: 'h' }).filename).toBe('unifed-remoteEntry.js')
...
describe('配置校验（DX：清晰报错）', () => {
  ...
  it('合法最小配置不被误杀：host + remote 双形态', () => {
    ...
    expect(host.remotes[0].devEntry).toContain('@unifed-entry.js')
