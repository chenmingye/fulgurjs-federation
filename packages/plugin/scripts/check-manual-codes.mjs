...
// 中的每个错误码必须在 README「错误码总表」有对应条目（README 为唯一权威文档），缺失则非零退出。
...
const manual = fs.readFileSync(path.join(root, 'README.md'), 'utf8')
...
if (missing.length) {
  console.error(`[fulgurjs] README 错误码总表缺少以下条目：${missing.join('、')}`)
  process.exit(1)
}
console.log(`[fulgurjs] 错误码总表一致（${codes.size} 个错误码全部有文档）`)
