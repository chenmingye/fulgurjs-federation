...
async function startRemote(): Promise<ReturnType<typeof spawn>> {
  ...
  for (let i = 0; i < 60; i++) {
    try {
      const res = await fetch(`http://localhost:${PORT}/@unifed-manifest.json`, { signal: AbortSignal.timeout(500) })
  ...
  test('kill remote → MFU-001 → 重启 → 恢复', async ({ page }) => {
    ...
    try {
      ...
      await page.evaluate(async (port) => {
        const rt = (window as any).__UNIFED_RUNTIME__
        rt.registerRemote({ name: 'remote-a-sa', entry: `http://localhost:${port}/@unifed-entry.js` })
      }, PORT)
      const ns1 = await page.evaluate(async () => {
        const m = await (window as any).__UNIFED_RUNTIME__.loadRemote('remote-a-sa/utils')
        return m.ANSWER
      })
      expect(ns1).toBe(42)
