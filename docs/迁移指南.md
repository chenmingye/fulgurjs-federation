...
> **目录约定说明**：插件自动生成的类型声明统一写在项目根 **`.fulgurjs/types/`**（0.5.9 起默认，src 零污染；tsconfig `include` 加一行 `".fulgurjs"`）。迁移工具脚手架生成的 `src/fulgurjs-exposes/`（联邦启动引导 + 暴露组件）与 `src/qiankun/fulgurjsBridge.ts` 等**属于本迁移工具的约定**，不是插件要求——你可以按团队习惯重组目录，只要 vite `exposes` 映射与 boot 调用点跟着改。
