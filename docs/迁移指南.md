...
| 2 | pnpm 装 tarball 软链断链 | `Cannot find module '@fulgur/federation'` | 重新 `pnpm add -D <tgz>` 并验证目录可达 |
| 3 | UMD/CJS 依赖被移出预构建 | dev 裸 CJS 白屏、`Cannot destructure property 'node'` | 放回 `optimizeDeps.include`（插件自动外部化 shared 键） |
| 4 | 给 shared 依赖加 ESM 别名 | 构建期 `xxx.default.extend is not a function` | 删别名（如 dayjs→dayjs/esm），插件统一处理 |
