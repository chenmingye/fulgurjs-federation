...
> **目录约定说明（0.6.0 起）**：联邦相关产物统一收在 **`src/fulgurjs/`** 一个文件夹——`types/`（插件每次 dev 自动生成的类型直连与运行时垫片，**勿手改**；src 布局 tsconfig 零配置生效，无 src 布局回退根目录 `.fulgurjs/types/`）与 `exposes/`（联邦启动引导 federatedBoot + 供外部加载的组件，**你的代码**）。宿主侧桥接文件（fulgurjsBridge/fulgurjsPages）同属迁移工具脚手架约定而非插件要求，可按团队习惯重组，只要 vite `exposes` 映射与调用点跟着改。
