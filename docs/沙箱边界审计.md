...
**联邦没有沙箱，也不需要沙箱**——这是与 qiankun 的根本架构差异。qiankun 用 JS Proxy 假 window + 样式隔离做「隔离共存」；@fulgur/federation（与 webpack MF 一致）做「**同 realm 共存 + 依赖级隔离**」：所有联邦模块与宿主跑在同一个 window/同一个 CSSOM 里，靠**共享依赖单例协商**防止最大的互扰源（双 Vue 运行时），而不是靠沙箱隔开。
...
- 联邦运行时唯一注册面：`window.__UNIFED_SCOPE__`（值 `['default']`）与 `__UNIFED_INFO__`——无全局散落
- **未发现子应用覆盖宿主关键全局**
