...
- 不在证据中写 token、Cookie、密码或带凭据 URL；业务数据、人员信息按现行规则脱敏。公开 Git 仓库只提交不含敏感信息的报告和经核验可公开的少量证据；完整截图与 HAR 保存在上述仓库外证据根，附索引与哈希。不要因截图未被 Git 跟踪就宣称“远端已保存全部证据”。
...
提交并推送**可公开的报告及必要脚本/脱敏摘要**到插件 Git；完整业务截图、HAR、令牌相关数据放仓库外证据根，不推公开仓库。若本轮只是测试与文档提交，不为此单独发布代码版本。最终报告必须给出用户可打开的本机截图路径，不能说“证据已上传 Git”而实际只在被删除的工作树中。只有下列条件同时成立，才可以写“本轮完整验收通过”：
