/**
 * demo（jeecg/yudao 三应用）联邦接入配置——fulgur init 验收样例（可入库、可复跑）。
 * 用法：npx fulgur init --config examples/demo-fulgur.config.ts
 * 前置：root 指向从 svn 原项目全新拷贝出的工程目录（CLI 在其上做零手工编辑集成）。
 */
import { defineFulgurConfig } from '@fulgur/federation/config'

export default defineFulgurConfig({
  root: '/Users/Admin/Desktop/ai 杂物/插件/fulgur-federation/testbed/demo-app',
  env: {
  ...
  apps: [
    {
      ...
      host: {
        ...
        pages: [
          ...
          // spec 显式覆盖：参数段剥离会把 expose 键错误收敛成列表页
    ...
    },
    {
      ...
      remote: {
        boot: 'bpm',
        remotes: { 'demo-host': { dev: 'http://localhost:8773/main', prod: '/main' } },
        detailPatch: true,
      },
    },
    {
